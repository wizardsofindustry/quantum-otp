...
class PinEndpoint(sq.interfaces.http.Endpoint):
    ...
    following functionality:
        ...
        Provides an interface to create or regenerate the Personal Identification Number (PIN) for the Subject identified in the request entity.


    A :class:`PinEndpoint` validates the structure of the request headers,
    URL parameters, query parameters and entity prior to forwarding the
    request to its handler (controller).

    The handler function (e.g., :meth:`~PinCtrl.get()`) may,
    instead of a :class:`~sq.interfaces.http.Response` object, return a tuple
    or a dictionary. The :class:`PinEndpoint` instance will interpret
    ...
    During serialization, A schema may be selected by :class:`PinEndpoint`
    based on the response status code and content type, if one was defined in
    the OpenAPI definition for this API endpoint.
    """
    pattern = "/generate/pin"
    ctrl = ioc.class_property("PinCtrl")
